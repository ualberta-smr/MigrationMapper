...
public class MigrationRuleDB {
    ...
    public LinkedList<MigrationRule> getMigrationRules() {
        ...
        try {
            ...
            while (rs.next()) {
                migrationRules.add(new MigrationRule(rs.getInt("ID"), rs.getString("FromLibrary"),
                        rs.getString("ToLibrary"), rs.getInt("Frequency"), rs.getDouble("Accuracy")));
            }
            rs.close();
            stmt.close();
            c.close();
    ...
    public LinkedList<MigrationRule> getMigrationRulesWithoutVersion(int isVaild) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String[] FromLibrarySP = getAppInfo(FromLibrary);
                String ToLibrary = rs.getString("ToLibrary");
                String[] ToLibrarySP = getAppInfo(ToLibrary);
    ...
    public LinkedList<MigrationRule> getMigrationRulesWithoutVersionClient() {
        LinkedList<MigrationRule> migrationRules = new LinkedList<MigrationRule>();
        // migrationRules.add(new MigrationRule(1, "log4j:log4j", "org.slf4j:slf4j-api",
        // 1, 1.0));
        // migrationRules.add(new MigrationRule(351,
        // "org.json:json","com.google.code.gson:gson",1, 1.0));
        // migrationRules.add(new
        // MigrationRule(13,"com.google.code.gson:gson","com.fasterxml.jackson.core:jackson-databind",1,
        // 1.0));
        // migrationRules.add(new MigrationRule(2231, "jmock:jmock", "junit:junit", 1,
        // 1.0));
        migrationRules.add(new MigrationRule(1, "commons-logging", "slf4j", 1, 1.0));
        // migrationRules.add(new MigrationRule(1, "log4j:log4j",
    ...
    int getMigrationRuleID(String FromLibrary, String ToLibrary) {
        ...
        try {
            ...
            c.setAutoCommit(false);
            stmt = c.createStatement();
            ResultSet rs = stmt.executeQuery("SELECT * FROM MigrationRules where FromLibrary='" + FromLibrary
                    + "' and ToLibrary='" + ToLibrary + "' ;");
    ...
    public void addMigrationRule(String FromLibrary, String ToLibrary, int Frequency, Double Accuracy) {
        ...
        try {
            ...
            c.setAutoCommit(false);
            stmt = c.createStatement();
            String sql = "INSERT INTO MigrationRules (FromLibrary,ToLibrary,Frequency,Accuracy) " + "VALUES ('"
                    + FromLibrary + "', '" + ToLibrary + "'," + Frequency + "," + Accuracy + ");";
