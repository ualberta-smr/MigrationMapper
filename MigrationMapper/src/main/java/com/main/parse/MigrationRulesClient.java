...
public class MigrationRulesClient {
    ...
    void start() throws IOException, InterruptedException {
        ...
        for (Project project : listOfProjectLibraries) {
            ...
            // System.out.println( "CommitID:"+project.CommitID
            ...
            if (oldcommitID.equals(project.CommitID) == false) {
                ...
                if (listOfAddedLibraries.size() > 0 && listOfRemovedLibraries.size() > 0) {
                    System.out.println("(" + currentLibraries + "-" + totalLibraries + ")--> Find CP between:"
                            + project.CommitID + "<==>" + oldcommitID);
                    if (isCommitsSequence(oldcommitID, newcommitID) == false) {
                        ...
                        listOfProjectLibrariesCP = cartesianProduct.generateProjectLibrariesCP(listOfProjectLibrariesCP,
                                listOfAddedLibraries, listOfRemovedLibraries);
                    }
        ...
        for (CPObject cpObject : listOfFilterLibraries) {
            if (cpObject.Accuracy >= threeShold && cpObject.Frequency >= frequency) {
                ...
                // System.err.println("Cannot download either "+ cpObject.value1+ " or "+
                // cpObject.value2 );
                // numberOfFalseRules++;
                // TODO: need to move process of downlaod jar to place of generate fragment
                // continue;
                // }
                String[] AppInfo = MigrationRuleDB.getAppInfo(cpObject.value1);
                String[] AppInfo2 = MigrationRuleDB.getAppInfo(cpObject.value2);
                if (AppInfo2[1].equals(AppInfo[1]) == false) {
                    migrations.append(cpObject.value1 + " <======> " + cpObject.value2 + "\t| frequency:"
