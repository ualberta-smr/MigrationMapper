...
import com.library.source.DownloadLibraryBase;
import com.library.source.DownloadPythonLibrary;
...
import com.segments.build.*;
import com.subversions.process.GitHubOP;

/**
 * This Client responsible for detect all fragments by cloning the apps from
 * GitHub
 */
public class DetectorClient {
    ...
    void start() {
        cleanCode = AppSettings.isPython() ? new CleanPythonCode() : new CleanJavaCode();
        ...
        // ArrayList<Repository> listOfRepositories= new
        // RepositoriesDB().getRepositories();
        // HashMap<Integer, LinkedList<AppCommit>> appsCommitsList=
        // appCommitsDB.getAppCommits();
        ...
        for (MigrationRule migrationRule : migrationRules) {
                ...
                if (oldcommitID.equals(newcommitID) == false) {
                    ...
                    if (listOfAddedLibraries.size() > 0 || listOfRemovedLibraries.size() > 0) {
                        ...
                        if (migrationRulesClient.isCommitsSequence(oldcommitID, newcommitID) == false) {
                            System.err.println("==>This process ingored because uncorrect order commits in between "
                            ...
                            String isFoundInPrevious = Project.isFound(listOfRemovedLibraries,
                                    ":" + migrationRule.ToLibrary);
                            if (isFoundInPrevious.length() > 0) {
                                continue;
                            }

                            String toLibraryFind = Project.isFound(listOfAddedLibraries, ":" + migrationRule.ToLibrary);
                            String fromLibraryFind = Project.isFound(listOfRemovedLibraries,
                                    ":" + migrationRule.FromLibrary);

                            // Case: for library that migrate but we didnot remove from the pom.xml
                            String isStillFound = Project.isFound(listOfCurrentProjectLibraries,
                                    ":" + migrationRule.FromLibrary);
                            if (toLibraryFind.length() > 0 && fromLibraryFind.length() == 0) {
                                fromLibraryFind = isStillFound;
                            }
                            // Case: use removed old library and he have new library already added in
                            // prevous commits
                            if (toLibraryFind.length() == 0 && fromLibraryFind.length() > 0) {
                                toLibraryFind = Project.isFound(listOfCurrentProjectLibraries,
                                        ":" + migrationRule.ToLibrary);
                            }
                            if (toLibraryFind.length() > 0 && fromLibraryFind.length() > 0) {
                                ...
                                if (previousCommitID.length() > 0) {
                                    ...
                                    ArrayList<Segment> listOfblocks = startCloning(appLink, previousCommitID,
                                            oldcommitID);
                                    ...
                                    if (listOfblocks.size() > 0) {
                                        ...
                                        for (Segment segment : listOfblocks) {
                                            ...
                                            migratedCommitDB.add(migrationRule.ID, currentProjectsID, oldcommitID,
                                                    segment, fromLibraryFind, toLibraryFind);
            ...
            ArrayList<Segment> listOfblocks = findSelfAdmittedMigration(migrationRule.ID, fromLibraryVaild,
                    toLibraryVaild, migrationRule.FromLibrary, migrationRule.ToLibrary);
            if (listOfblocks.size() > 0) {
                segmentList.addAll(listOfblocks);
            }
    ...
    ArrayList<Segment> findSelfAdmittedMigration(int migrationRuleID, String fromLibraryVaild, String toLibraryVaild,
                                                 String fromLibraryName, String toLibraryName) {
        ArrayList<Segment> segmentList = new ArrayList<Segment>();

        if (fromLibraryVaild == null || toLibraryVaild == null) {
            System.err.println(
                    "Either fromLibrary=" + fromLibraryVaild + "  or toLibrary=" + toLibraryVaild + " is not vaild");
            return segmentList;
        }
        AppCommitsDB appCommitsDB = new AppCommitsDB();
        MigrationSegmentsDB migratedCommitDB = new MigrationSegmentsDB();
        ArrayList<AppCommit> listOfAppsCommit = appCommitsDB.getAllCommitsHasMigration(fromLibraryName, toLibraryName,
                migrationRuleID);

        System.out.println("Start searching for self-admitted migration between " + fromLibraryVaild + "==>"
                + toLibraryVaild + " by the developer");
                ...
                if (listOfblocks.size() > 0) {
                    ...
                    for (Segment segment : listOfblocks) {
                        ...
                        migratedCommitDB.add(MigratedLibraries.ID, appCommit.AppID, appCommit.CommitID, segment,
                                fromLibraryVaild, toLibraryVaild);
    ...
    ArrayList<Segment> startCloning(String appURL, String previousCommitName, String migrateAtCommitName) {
        ...
        if (listOfChangedFiles.size() > 0) {
            ...
            ArrayList<String> diffsFilePath = generateFragments(listOfChangedFiles, previousCommitName,
                    migrateAtCommitName, outputDiffsPath, appURL);

            if (diffsFilePath.size() > 0) {
                // clean java code from the data
                segmentList = cleanCode.getListOfCleanedFiles(outputDiffsPath, diffsFilePath);
            }
    ...
    ArrayList<String> cloneMigratedCommits(String appURL, String previousCommitName, String migrateAtCommitName) {
        ArrayList<String> listOfChangedFiles = new ArrayList<String>();

        // Download The library Jar singatures
        DownloadLibraryBase downloadLibrary = AppSettings.isPython() ?
                new DownloadPythonLibrary(pathToSaveJAVALibrary) :
                new DownloadLibrary(pathToSaveJAVALibrary);
    ...
    ArrayList<String> generateFragments(ArrayList<String> listOfChangedFiles, String previousCommitName,
        ...
        for (String changedFilePath : listOfChangedFiles) {
            ...
            if (cleanCode.isUsedNewLibrary(oldFilePath, MigratedLibraries.fromLibrary)
                    && cleanCode.isUsedNewLibrary(newUpdatedFilePath, MigratedLibraries.toLibrary)) {
                ...
                GitHubOP gitHubOP = new GitHubOP(appURL, pathClone);
                gitHubOP.createDiffs(previousCommitName, migrateAtCommitName, changedFilePath, outputDiffFilePath);
